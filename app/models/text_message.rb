class TextMessage < ActiveRecord::Base
  ...
  def self.secret_code_find(the_user, the_code)
  	...
  	code_found = false
  	user_to_return = nil
  	refined_texts = TextMessage.select{|x| x.user_id != the_user.id and x.secret_code != nil}
  ...
  def self.does_another_message_exist(text_message_content, user_id)
  	initial_map = TextMessage.all.select{|x| !x.secret_code.nil? and x.user_id != user_id}
